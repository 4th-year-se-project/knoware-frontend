...
export const getAllComments = async (
  ...
  try {
    const response = await axios.get(
      `http://localhost:8080/get_all_comments?document_id=${doc_id}&embedding_id=${embedding_id}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.access_token}`,
        },
...
export const addEmbeddingComment = async (
  ...
  try {
    const response = await axios.post(
      `http://localhost:8080/add_embedding_comment?document_id=${doc_id}&embedding_id=${embedding_id}&comment=${comment}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.access_token}`,
        },
...
export const getAllCoursess = async (): Promise<AxiosResponse> => {
  try {
    const response = await axios.get(`http://localhost:8080/get-all-courses`, {
      headers: {
        Authorization: `Bearer ${localStorage.access_token}`,
      },
    });
...
export const editResourceLabel = async (
  document_id: number,
  label: string | null
): Promise<AxiosResponse> => {
  try {
    const response = await axios.put(
      `http://localhost:8080/update-label?document_id=${document_id}&label=${label}`
    );
    return response;
  } catch (error) {
    throw error;
